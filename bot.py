...
def get_candles(pair):
    r = requests.get(
        BASE_URL + f"/api/v5/market/candles?instId={pair}&bar=1H&limit=100"
    ).json()["data"]
...
def has_position(pair):
    r = requests.get(
        BASE_URL + f"/api/v5/account/positions?instId={pair}",
        headers=headers("GET", f"/api/v5/account/positions?instId={pair}")
    ).json()
